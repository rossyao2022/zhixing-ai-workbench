...
const source = {
  ...
  clickBook: {
    label: "Character · Click / Foundation Sprint",
    url: "https://www.character.vc/click",
  },
  userStoryMapping: {
    label: "O’Reilly · User Story Mapping",
  ...
  productBookLab: {
    label: "夸夸学习 AI · 第三课四本书可视化任务工作台",
    url: "/kuakua-ai/slides/lesson-03-mvp-experiment/book-lab.html",
  },
  productLesson60: {
    label: "夸夸学习 AI · 第三课 60 分钟核心授课版",
    url: "/kuakua-ai/slides/lesson-03-mvp-experiment/index.html?mode=60",
  },
  designKit: {
...
export const courseStages: CourseStage[] = [
  ...
  },
  {
    ...
    title: "产品定义与最小验证",
    subtitle: "用四本核心书完成定假设、画全程、设边界和做实验，把洞察变成可开工、可验证的产品下注。",
    weeks: "第 4–5 周",
    color: "#6541c9",
    outcome: "让真实用户用行为证明产品是否值得继续。",
    deliverable: "四本书可视化任务包 + 产品需求简报 + 可点击原型 + 首轮验证结论",
    lessons: [
      {
        id: "product-01",
        title: "从客户任务写出价值主张",
        duration: "30 分钟",
        xp: 55,
        summary: "先用 Click 的 Foundation Hypothesis 罗盘写清客户、问题、现有替代、独特优势、产品路径和差异，再把客户任务、阻力与收益连成单一价值承诺。",
        objectives: ["完成七格创始假设罗盘", "为每个判断标注证据来源", "形成可被陌生人复述和挑战的核心承诺"],
        practice: "进入四本书工作台完成 Click 罗盘；删除所有不能对应访谈或行为证据的判断，把最大未知留给后续实验。",
        deliverable: "Foundation Hypothesis 罗盘 + 一句话价值主张",
        sources: [source.clickBook, source.strategyzerProfile, source.jasonNeeds, source.productBookLab, source.productLesson60],
      },
      {
        id: "product-02",
        ...
        duration: "34 分钟",
        xp: 60,
        summary: "把 Lean Startup 的开发—测量—认知作为外层学习循环，再用 Testing Business Ideas 锁定最高影响、最低证据的判断；缩小范围，但完整完成一个客户任务。",
        objectives: ["识别价值、可用性、可行性、商业四类风险", "按影响与证据给假设排序", "预写实验指标、样本与通过/观察/失败阈值"],
        practice: "进入四本书工作台锁定最高风险假设，生成一张 7 天 Test Card；删掉一半范围，但保留端到端用户结果，并提前写下停止条件。",
        deliverable: "风险矩阵 + Test Card + MVP 实验卡",
        sources: [source.testingBusinessIdeas, source.strategyzerTest, source.ycEssential, source.jasonSlc, source.gettingReal, source.productBookLab],
      },
      {
        id: "product-03",
        ...
        duration: "32 分钟",
        xp: 55,
        summary: "用 User Story Mapping 从触发到结果铺开用户活动与步骤，通过横向切片切出虽小但端到端完整的 Release 1。",
        objectives: ["定义并标记首个价值时刻", "绘制、排序主流程与异常路径", "区分 Release 1 与 Later 且不切断用户结果"],
        practice: "进入四本书工作台新增、拖动和排序步骤，把主流程压缩到 5 个关键步骤，标记首个价值时刻，并解释每一步为什么不能删。",
        deliverable: "可交互用户故事地图 + Release 1 切片",
        sources: [source.userStoryMapping, source.designKit, source.nngHeuristics, source.productBookLab],
      },
      {
        id: "product-04",
        title: "写一份能直接开工的产品简报",
        duration: "38 分钟",
        xp: 70,
        summary: "用 Shape Up 固定时间胃口与人数，从故事地图同步首发范围，再把问题、粗粒度解法、风险洞和不做项放进同一份可下注的决策文档。",
        objectives: ["用 appetite 而非无限估时约束范围", "检查首发切片是否超出可用容量", "写清 Rabbit Holes、No-gos 与验收条件"],
        practice: "进入四本书工作台同步 Release 1，调整时间胃口和人数，生成一页 Pitch；邀请未参与者复述并记录所有理解偏差。完成核心课后，再把《设计冲刺》作为五日原型进阶。",
        deliverable: "可视化容量边界 + 一页 Shape Up Pitch / PRD Lite",
        sources: [source.shapeUpBook, source.shapeBoundaries, source.shapeBets, source.analytics, source.doubleDiamond, source.productBookLab, source.productLesson60],
      },
    ],
  },
  {
