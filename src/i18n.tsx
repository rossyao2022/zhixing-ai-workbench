...
const messages: Record<string, MessageTuple> = {
  ...
  "dashboard.praiseEvidence": ["你刚完成「{title}」，这不是看懂，而是已经能应用。", "You completed “{title}”. You did not just understand it—you applied it.", "Completaste «{title}». No solo lo entendiste: ya lo aplicaste.", "आपने “{title}” पूरा किया। आपने सिर्फ समझा नहीं—लागू भी किया।", "أنجزت «{title}». لم تكتفِ بالفهم، بل طبّقته فعلاً."],
  "dashboard.praisePhase": ["你已经把「{title}」推进到“{phase}”，今天只要再完成下一小步。", "You moved “{title}” to “{phase}”. Today, finish just the next small step.", "Llevaste «{title}» hasta «{phase}». Hoy completa solo el siguiente paso pequeño.", "आप “{title}” को “{phase}” तक ले आए। आज बस अगला छोटा कदम पूरा करें।", "قدّمت «{title}» إلى مرحلة «{phase}». أكمل اليوم الخطوة الصغيرة التالية فقط."],
  "dashboard.praiseProgress": ["你已经留下 {count} 份真实作品证据，能力正在变成可复用的资产。", "You have created {count} pieces of real evidence. Your ability is becoming a reusable asset.", "Ya creaste {count} evidencias reales. Tu capacidad se está convirtiendo en un activo reutilizable.", "आपने {count} वास्तविक प्रमाण बनाए हैं। आपकी क्षमता पुनः उपयोग योग्य संपत्ति बन रही है।", "أنشأت {count} أدلة حقيقية، وقدرتك تتحول إلى أصل قابل لإعادة الاستخدام."],
  "dashboard.praiseStreak": ["连续 {count} 天回来推进，你正在建立比灵感更可靠的节奏。", "You returned for {count} days. You are building a rhythm more reliable than inspiration.", "Volviste durante {count} días. Estás creando un ritmo más fiable que la inspiración.", "आप {count} दिन लौटे। आप प्रेरणा से अधिक भरोसेमंद लय बना रहे हैं।", "عدت {count} أيام متتالية، وتبني إيقاعاً أوثق من الإلهام."],
  "dashboard.praiseWelcome": ["你愿意把想法带进真实练习，这已经是成为超级个体的第一步。", "Bringing an idea into real practice is already the first step toward a solo company.", "Llevar una idea a la práctica real ya es el primer paso hacia una empresa individual.", "विचार को वास्तविक अभ्यास में लाना ही एकल कंपनी की पहली सीढ़ी है।", "إدخال فكرتك في تطبيق حقيقي هو بالفعل الخطوة الأولى نحو شركة فردية."],
  "dashboard.focusTitle": ["今天，只推进这一个成果", "Today, move one outcome forward", "Hoy, avanza un solo resultado", "आज केवल एक परिणाम आगे बढ़ाएँ", "اليوم، قدّم نتيجة واحدة فقط"],
  "dashboard.focusComplete": ["32 课的作品证据已经连成一套完整的个人公司系统。", "Your 32 lesson outputs now form a complete solo-company system.", "Tus 32 resultados forman ya un sistema completo de empresa individual.", "आपके 32 पाठ परिणाम अब संपूर्ण एकल-कंपनी प्रणाली हैं।", "أصبحت مخرجات الدروس الـ32 نظام شركة فردية متكاملاً."],
  "dashboard.resumePhase": ["继续：{phase}", "Continue: {phase}", "Continuar: {phase}", "जारी रखें: {phase}", "تابع: {phase}"],
  "dashboard.startStep": ["开始今天这一步", "Start today's step", "Empieza el paso de hoy", "आज का कदम शुरू करें", "ابدأ خطوة اليوم"],

  "learning.loopTitle": ["理解、学透、练会、做出来", "Understand, learn, practise, ship", "Entiende, aprende, practica y crea", "समझें, सीखें, अभ्यास करें, बनाएँ", "افهم وتعلّم وتدرّب وأنجز"],
  "learning.phase.concept": ["核心概念", "Core idea", "Idea central", "मुख्य विचार", "الفكرة الأساسية"],
  "learning.phase.conceptHint": ["一句话抓住本质", "Grasp the essence", "Capta la esencia", "सार समझें", "استوعب الجوهر"],
  "learning.phase.learn": ["学透方法", "Learn deeply", "Aprende a fondo", "गहराई से सीखें", "تعلّم بعمق"],
  "learning.phase.learnHint": ["方法、案例与课程资料", "Method, case and resources", "Método, caso y recursos", "तरीका, केस और संसाधन", "منهج وحالة وموارد"],
  "learning.phase.practice": ["AI 陪练", "AI practice", "Práctica con IA", "AI अभ्यास", "تدريب بالذكاء"],
  "learning.phase.practiceHint": ["用真实材料获得反馈", "Feedback on real material", "Feedback sobre material real", "वास्तविक सामग्री पर प्रतिक्रिया", "ملاحظات على مادة حقيقية"],
  "learning.phase.workbench": ["动手工作台", "Hands-on workbench", "Taller práctico", "व्यावहारिक कार्यस्थल", "مساحة العمل التطبيقية"],
  "learning.phase.workbenchHint": ["为所选文件生成限定任务单", "Create a bounded task for selected files", "Crea una tarea limitada para archivos elegidos", "चुनी फ़ाइलों के लिए सीमित कार्य बनाएँ", "أنشئ مهمة محددة للملفات المختارة"],
  "learning.back": ["返回课程", "Back to course", "Volver al curso", "पाठ्यक्रम पर लौटें", "العودة للدورة"],
  "learning.phaseNav": ["四步学习进度", "Four-step lesson progress", "Progreso de cuatro pasos", "चार-चरण पाठ प्रगति", "تقدم الدرس في أربع خطوات"],
  "learning.previous": ["上一步", "Previous", "Anterior", "पिछला", "السابق"],
  "learning.next": ["下一步", "Next step", "Siguiente", "अगला कदम", "الخطوة التالية"],
  "learning.caseSituation": ["情境", "Situation", "Situación", "स्थिति", "الموقف"],
  "learning.caseAnalysis": ["分析", "Analysis", "Análisis", "विश्लेषण", "التحليل"],
  "learning.caseDecision": ["决定", "Decision", "Decisión", "निर्णय", "القرار"],
  "learning.coachPrivacyTitle": ["材料会发送给 DeepSeek。", "Your material is sent to DeepSeek.", "Tu material se envía a DeepSeek.", "आपकी सामग्री DeepSeek को भेजी जाती है।", "تُرسل مادتك إلى DeepSeek."],
  "learning.coachPrivacy": ["请先去除姓名、电话、密钥和客户机密；平台不保存材料、提示词或模型回答。", "Remove names, phone numbers, keys and client secrets first. The platform does not store materials, prompts or model answers.", "Quita nombres, teléfonos, claves y secretos del cliente. La plataforma no guarda materiales, prompts ni respuestas.", "पहले नाम, फोन, कुंजियाँ और ग्राहक रहस्य हटाएँ। प्लेटफ़ॉर्म सामग्री, प्रॉम्प्ट या उत्तर सहेजता नहीं है।", "احذف الأسماء والهواتف والمفاتيح وأسرار العملاء أولاً. لا تحفظ المنصة المواد أو التعليمات أو الإجابات."],
  "learning.coachMaterial": ["粘贴一份真实项目材料", "Paste real project material", "Pega material real del proyecto", "वास्तविक प्रोजेक्ट सामग्री चिपकाएँ", "ألصق مادة حقيقية من مشروعك"],
  "learning.coachPlaceholder": ["写清当前情境、已有证据、限制、草稿和你正在做的决定。请先匿名化。", "State the situation, evidence, constraints, draft and decision. Anonymise first.", "Describe la situación, evidencia, límites, borrador y decisión. Anonimiza primero.", "स्थिति, प्रमाण, सीमाएँ, मसौदा और निर्णय लिखें। पहले पहचान हटाएँ।", "اكتب الموقف والدليل والقيود والمسودة والقرار، بعد إخفاء الهوية."],
  "learning.minimum": ["最少字符", "minimum characters", "caracteres mínimos", "न्यूनतम अक्षर", "الحد الأدنى للحروف"],
  "learning.coachChecks": ["本次陪练会检查", "This coach checks", "El coach comprobará", "यह कोच जाँचेगा", "سيتحقق المدرب من"],
  "learning.coachRunning": ["DeepSeek 正在分析…", "DeepSeek is analysing…", "DeepSeek está analizando…", "DeepSeek विश्लेषण कर रहा है…", "يحلل DeepSeek الآن…"],
  "learning.coachStart": ["开始一轮陪练", "Start coaching", "Empezar práctica", "अभ्यास शुरू करें", "ابدأ التدريب"],
  "learning.strengths": ["已经做对", "What works", "Lo que funciona", "क्या सही है", "ما تم جيداً"],
  "learning.gaps": ["还需补齐", "What is missing", "Lo que falta", "क्या कमी है", "ما ينقص"],
  "learning.improvedDraft": ["参考改写", "Suggested revision", "Revisión sugerida", "सुझाया संशोधन", "صياغة مقترحة"],
  "learning.oneQuestion": ["只回答这一个问题", "Answer just this question", "Responde solo esta pregunta", "केवल इस प्रश्न का उत्तर दें", "أجب عن هذا السؤال فقط"],
  "learning.nextAction": ["下一步动作", "Next action", "Siguiente acción", "अगली कार्रवाई", "الإجراء التالي"],
  "learning.harnessTitle": ["把方法带进你的真实文件", "Take the method into your real files", "Lleva el método a tus archivos reales", "तरीके को वास्तविक फ़ाइलों में ले जाएँ", "طبّق المنهج على ملفاتك الحقيقية"],
  "learning.harnessIntro": ["选择本地文件，生成一份边界明确的任务单；网页不会读取或上传文件内容，由你在本机 Harness 中确认每次修改。", "Select local files and create a bounded task. The site never reads or uploads file content; you approve edits in your local Harness.", "Selecciona archivos y crea una tarea limitada. El sitio no lee ni sube contenido; tú apruebas los cambios en Harness local.", "स्थानीय फ़ाइलें चुनकर सीमित कार्य बनाएँ। साइट सामग्री नहीं पढ़ती या अपलोड करती; बदलाव स्थानीय Harness में आप मंजूर करते हैं।", "حدد ملفات محلية وأنشئ مهمة محددة. لا يقرأ الموقع المحتوى ولا يرفعه؛ وتؤكد التعديلات في Harness المحلي."],
  "learning.harnessSafetyTitle": ["先保护工作区", "Protect the workspace first", "Protege primero el espacio", "पहले कार्यक्षेत्र सुरक्षित करें", "احمِ مساحة العمل أولاً"],
  "learning.harnessSafety": ["使用专用练习副本；不要选择 .env、私钥或客户秘密；保持 workspace-write 与逐次确认，提交前查看 diff。", "Use a dedicated practice copy. Never select .env files, private keys or client secrets. Keep workspace-write and approval on; review the diff before committing.", "Usa una copia de práctica. No elijas .env, claves privadas ni secretos. Mantén workspace-write y aprobación; revisa el diff.", "अभ्यास की अलग प्रति उपयोग करें। .env, निजी कुंजी या ग्राहक रहस्य न चुनें। workspace-write व मंजूरी रखें; diff देखें।", "استخدم نسخة تدريبية مستقلة. لا تحدد ملفات .env أو المفاتيح أو الأسرار. أبقِ workspace-write والتأكيد وراجع الفروق."],
  "learning.selectFiles": ["选择文件", "Select files", "Elegir archivos", "फ़ाइलें चुनें", "حدد ملفات"],
  "learning.selectFolder": ["选择文件夹", "Select folder", "Elegir carpeta", "फ़ोल्डर चुनें", "حدد مجلداً"],
  "learning.fileLocalOnly": ["这里只记录文件名、相对路径和大小；不会读取内容，也不会发起本地连接。", "Only names, relative paths and sizes are listed. No content is read and no local connection is made.", "Solo se listan nombres, rutas y tamaños. No se lee contenido ni se conecta localmente.", "केवल नाम, सापेक्ष पथ और आकार सूचीबद्ध हैं। सामग्री नहीं पढ़ी जाती और कोई स्थानीय कनेक्शन नहीं होता।", "تُعرض الأسماء والمسارات والأحجام فقط؛ لا يُقرأ المحتوى ولا يبدأ اتصال محلي."],
  "learning.workspacePath": ["本地工作区路径（可选）", "Local workspace path (optional)", "Ruta local (opcional)", "स्थानीय कार्यक्षेत्र पथ (वैकल्पिक)", "مسار العمل المحلي (اختياري)"],
  "learning.workspacePathHint": ["仅写入任务单，网页不访问这个路径。", "Included only in the task; the site does not access it.", "Solo se incluye en la tarea; el sitio no accede a ella.", "केवल कार्य में शामिल; साइट इसे नहीं खोलती।", "يُدرج في المهمة فقط؛ ولا يصل إليه الموقع."],
  "learning.selectedFiles": ["允许修改的文件", "Files allowed to change", "Archivos permitidos", "बदलने की अनुमति वाली फ़ाइलें", "الملفات المسموح تعديلها"],
  "learning.secretExcluded": ["已从任务单排除 {count} 个疑似敏感文件。", "Excluded {count} potentially sensitive files from the task.", "Se excluyeron {count} archivos sensibles.", "{count} संभावित संवेदनशील फ़ाइलें कार्य से हटाई गईं।", "تم استبعاد {count} ملفات يحتمل أن تكون حساسة."],
  "learning.harnessNoFiles": ["尚未选择文件", "No files selected", "No hay archivos", "कोई फ़ाइल नहीं चुनी", "لم تُحدد ملفات"],
  "learning.harnessTaskTitle": ["夸夸学习 AI · 本地实作任务单", "KuaKua Learning AI · Local task", "KuaKua Learning AI · Tarea local", "KuaKua Learning AI · स्थानीय कार्य", "KuaKua Learning AI · مهمة محلية"],
  "learning.harnessWorkspace": ["工作区", "Workspace", "Espacio", "कार्यस्थल", "مساحة العمل"],
  "learning.harnessChooseInApp": ["在 Harness 中手动选择", "Choose manually in Harness", "Elegir en Harness", "Harness में चुनें", "اختر يدوياً في Harness"],
  "learning.harnessLesson": ["课程", "Lesson", "Lección", "पाठ", "الدرس"],
  "learning.harnessGoal": ["实作目标", "Practice goal", "Objetivo", "अभ्यास लक्ष्य", "هدف التطبيق"],
  "learning.harnessOutput": ["验收成果", "Accepted output", "Resultado", "स्वीकृत परिणाम", "المخرج المطلوب"],
  "learning.harnessAllowedFiles": ["只允许修改以下所选文件：", "Only these selected files may be edited:", "Solo se pueden editar estos archivos:", "केवल ये चुनी फ़ाइलें बदली जा सकती हैं:", "لا تعدّل إلا الملفات المحددة التالية:"],
  "learning.harnessRules": ["执行规则：", "Execution rules:", "Reglas:", "निष्पादन नियम:", "قواعد التنفيذ:"],
  "learning.harnessRuleInspect": ["先只读检查相关文件和现有约束，不做修改。", "Inspect the files and constraints read-only before editing.", "Inspecciona archivos y límites antes de editar.", "बदलने से पहले फ़ाइलें और सीमाएँ केवल पढ़ें।", "افحص الملفات والقيود للقراءة فقط قبل التعديل."],
  "learning.harnessRulePlan": ["给出最小修改计划和验收方法。", "Give the smallest edit plan and acceptance check.", "Propón el cambio mínimo y cómo validarlo.", "न्यूनतम बदलाव योजना और जाँच दें।", "قدّم أصغر خطة تعديل وطريقة تحقق."],
  "learning.harnessRuleEdit": ["只修改清单内文件；需要其他文件时先停下说明。", "Edit only listed files; stop and explain if another file is needed.", "Edita solo los archivos listados; detente si necesitas otro.", "केवल सूचीबद्ध फ़ाइलें बदलें; अन्य चाहिए तो रुकें।", "عدّل الملفات المذكورة فقط، وتوقف إن احتجت غيرها."],
  "learning.harnessRuleVerify": ["运行与修改范围相称的验证，不执行破坏性命令。", "Run proportionate checks and no destructive commands.", "Ejecuta comprobaciones proporcionales y nada destructivo.", "उचित जाँच चलाएँ, विनाशकारी कमांड नहीं।", "نفّذ تحققاً مناسباً بلا أوامر مدمرة."],
  "learning.harnessRuleDiff": ["最后总结修改、验证结果和 diff，等待我确认。", "Summarise edits, checks and diff, then wait for approval.", "Resume cambios, pruebas y diff, y espera aprobación.", "अंत में बदलाव, जाँच और diff बताएँ और मंजूरी की प्रतीक्षा करें।", "لخّص التعديلات والتحقق والفروق ثم انتظر موافقتي."],
  "learning.harnessTask": ["任务单预览", "Task preview", "Vista de tarea", "कार्य पूर्वावलोकन", "معاينة المهمة"],
  "learning.copyTask": ["复制任务单", "Copy task", "Copiar tarea", "कार्य कॉपी करें", "انسخ المهمة"],
  "learning.copied": ["已复制", "Copied", "Copiado", "कॉपी हुआ", "تم النسخ"],
  "learning.launchHarness": ["启动本地 DeepSeek Harness", "Start local DeepSeek Harness", "Iniciar DeepSeek Harness local", "स्थानीय DeepSeek Harness शुरू करें", "شغّل DeepSeek Harness المحلي"],
  "learning.launchHint": ["先在终端启动，再由你主动打开本地页面并选择工作区。", "Start it in a terminal, then open the local page yourself and choose the workspace.", "Inícialo en terminal, abre la página local y elige el espacio.", "टर्मिनल में शुरू करें, फिर स्थानीय पेज खोलकर कार्यस्थल चुनें।", "شغّله في الطرفية ثم افتح الصفحة المحلية واختر مساحة العمل."],
  "learning.copyCommand": ["复制启动命令", "Copy command", "Copiar comando", "कमांड कॉपी करें", "انسخ الأمر"],
  "learning.openHarness": ["打开本地工作台", "Open local workbench", "Abrir taller local", "स्थानीय कार्यस्थल खोलें", "افتح مساحة العمل المحلية"],
  ...
  "lesson.masterLanguageNotice": ["", "Language note: the interface and core method are translated; detailed cases, books, videos and source notes remain in the Simplified Chinese master edition.", "Nota de idioma: la interfaz y el método central están traducidos; los casos, libros, vídeos y fuentes detallados permanecen en la edición maestra en chino simplificado.", "भाषा सूचना: इंटरफ़ेस और मुख्य विधि अनूदित हैं; विस्तृत केस, पुस्तकें, वीडियो और स्रोत नोट सरलीकृत चीनी मास्टर संस्करण में हैं।", "ملاحظة اللغة: تُرجمت الواجهة والمنهج الأساسي؛ وتبقى الحالات والكتب والفيديوهات والمصادر المفصلة في النسخة الصينية المبسطة."],
