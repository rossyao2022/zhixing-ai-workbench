...
const outputDir = process.env.LESSON1_OUTPUT_DIR || path.resolve(root, "..", "outputs", "lesson1-feishu-delivery");
const manifestPath = path.join(outputDir, "slide-manifest.json");
const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));

const pptxPath = path.join(outputDir, "晴幂科技_第1课_AI与超级个体商业模式画布_完整版35页.pptx");
const pdfPath = path.join(outputDir, "晴幂科技_第1课_AI与超级个体商业模式画布_完整版35页.pdf");
const contactSheetPath = path.join(outputDir, "晴幂科技_第1课_35页总览.png");
...
async function buildPdf() {
  ...
  pdf.setAuthor("晴幂科技");
  pdf.setSubject("夸夸学习 AI 第一课完整课件");
...
async function buildContactSheet() {
  ...
  for (let i = 0; i < manifest.slides.length; i += 1) {
    const image = await sharp(manifest.slides[i].image)
      .resize(thumbWidth, thumbHeight, { fit: "fill" })
      .png()
      .toBuffer();
    composites.push({
      input: image,
      left: gap + (i % cols) * (thumbWidth + gap),
      top: gap + Math.floor(i / cols) * (thumbHeight + gap),
    });
  }
  await sharp({
